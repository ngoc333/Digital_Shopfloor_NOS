...
namespace FORM
{
    public partial class FORM_SMT_ASSEMBLY_INPUT_SET : Form
    {
        public FORM_SMT_ASSEMBLY_INPUT_SET()
        {
            InitializeComponent();
        }
        ...
        private void load_data(string arg_type)
        {
            try
            {
                DataTable dt = SEL_SMT_ASSEMBLY_INPUT_SET(arg_type, line, mline, "");
        ...
        private void formatgrid1()
        {
            try
            {
                ...
                gvwMain.Columns[0].Width = 95;
                gvwMain.Columns[1].Width = 100;
                gvwMain.Columns[2].Width = 220;
                gvwMain.Columns[3].Width = 120;
        ...
        public DataTable SEL_SMT_ASSEMBLY_INPUT_SET(string ARG_QTYPE, string ARG_LINE, string ARG_MLINE, string ARG_DATE)
        {
        ...
        private void initForm()
        {
            ...
            {
                case "Vn":
                    lblTitle.Text = "Lắp ráp Input Set";
                    break;
                case "En":
                    lblTitle.Text = "Assembly Input Set Balance";
                    break;
            }
        ...
        {
            if (gvwMain.GetRowCellValue(e.RowHandle, "Division").ToString().ToUpper().Equals("SET") && e.Column.ColumnHandle > _start_column - 3)
            {
                e.Appearance.BackColor = Color.FromArgb(255, 224, 255, 255);
                e.Appearance.ForeColor = Color.FromArgb(255, 255, 127, 80);
            }
            if (gvwMain.GetRowCellValue(e.RowHandle, "Plant").ToString().ToUpper().Equals("TOTAL"))
            {                                
