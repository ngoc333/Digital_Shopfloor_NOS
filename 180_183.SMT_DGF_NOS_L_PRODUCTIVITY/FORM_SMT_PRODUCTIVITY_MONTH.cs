...
namespace FORM
{
    public partial class FORM_SMT_PRODUCTIVITY_MONTH : Form
    {
        
    

        public FORM_SMT_PRODUCTIVITY_MONTH()
        {
            InitializeComponent();
        }
        [DllImport("user32")]
       ...
       public FORM_SMT_PRODUCTIVITY_MONTH(string Title, int _indexScreen, string line_cd, string mline_cd,string _Lang)
        {
            ...
            try
            {
                if (arg_dt != null && arg_dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < arg_dt.Rows.Count; i++)
                    {
                        ...
                        axGrid.SetText(icol, 10, arg_dt.Rows[i]["POH"].ToString());
                        axGrid.SetText(icol, 11, arg_dt.Rows[i]["POD"].ToString());
        ...
        {
            System.Data.DataSet ds = LOAD_DATA_v2(_wh_cd, _mline_cd, UC_MONTH.GetValue());
            axGrid.Hide();
            ...
            switch (Lang)
            {
                case "Vn":
                    ...
                    CreateChartLine(Chart2, ds.Tables[3], "May " + (1 + (3 * (iMline - 1))).ToString());
                    CreateChartLine(Chart3, ds.Tables[5], "May " + (2 + (3 * (iMline - 1))).ToString());
                    CreateChartLine(Chart6, ds.Tables[7], "May " + (3 + (3 * (iMline - 1))).ToString());
               ...
               break;
                case "En":
                    ...
                    CreateChartLine(Chart2, ds.Tables[3], "Stitching " + (1 + (3 * (iMline - 1))).ToString());
                    CreateChartLine(Chart3, ds.Tables[5], "Stitching " + (2 + (3 * (iMline - 1))).ToString());
                    CreateChartLine(Chart6, ds.Tables[7], "Stitching " + (3 + (3 * (iMline - 1))).ToString());
        ...
        public System.Data.DataSet LOAD_DATA_v2(string sLine_cd, string sMline_cd, string smonth)
        {
            ...
            try
            {
                string process_name = "MES.PKG_SMT_PROD_SHOW.SEL_PRODUCTIVITY_MONTH_L";
                ...
                MyOraDB.Parameter_Name[10] = "OUT_CURSOR7";
        ...
        private void initForm()
        {
            ...
            switch (Lang)
            {
                case "Vn":
                    ...
                    btnWeek.Text = "Tuần";
                    btnYear.Text = "Năm";
                    lblTitle.Text = "Trạng thái năng suất theo Tháng";
                    break;
                case "En":
                    btnDay.Text = "Day";
                    btnMonth.Text = "Month";
