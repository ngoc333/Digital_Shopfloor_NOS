...
namespace FORM
{
    public partial class FORM_QUALITY_RATE_MONTH : Form
    {
        ...
        public DataTable SP_SMT_FTT_MONTHLY(string ARG_QTYPE, string ARG_MONTH, string ARG_LINE_CD, string ARG_MLINE_CD)
        {
            ...
            try
            {
                string process_name = "MES.PKG_SMT_FTT_DAILY.SP_SMT_FTT_DAILY_NOS_L";
        ...
        private void BindingGrid()
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                try
                {
                    ...
                    axfpFTT.SetText(2, 3, "Sitching " + (1 + (3 * (iMline - 1))).ToString());
                    axfpFTT.SetText(2, 4, "Sitching " + (2 + (3 * (iMline - 1))).ToString());
                    axfpFTT.SetText(2, 5, "Sitching " + (3 + (3 * (iMline - 1))).ToString());
