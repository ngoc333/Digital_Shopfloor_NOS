...
namespace FORM
{
    public partial class FORM_INT_OSD : Form
    {
        public FORM_INT_OSD()
        {
            InitializeComponent();
        ...
        int cCount = 0;
        public FORM_INT_OSD(string Caption, int indexScreen, string Line_cd, string Mline_cd, string Lang)
        {
            InitializeComponent();
            timer1.Stop();
        ...
        private void UC_MONTH_ValueChangeEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                string DATE_FROM = UC_MONTH.GetValue() + "01", DATE_TO = UC_MONTH.GetValue() + DateTime.DaysInMonth(Convert.ToInt32(UC_MONTH.GetValue().Substring(0, 4)), Convert.ToInt32(UC_MONTH.GetValue().Substring(4, 2))).ToString();
                loadTopLeft(SEL_DATA_OSD("CHART_"+Mline, DATE_FROM, DATE_TO)); bindingDataGrid(DATE_FROM, DATE_TO);
                BindingChart();
