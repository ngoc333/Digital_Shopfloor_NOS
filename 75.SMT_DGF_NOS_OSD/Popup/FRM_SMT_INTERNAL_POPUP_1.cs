...
namespace FORM.Popup
{
    public partial class FRM_SMT_INTERNAL_POPUP_1 : Form
    {
        ...
        private void bgGrdView_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                if (!bgGrdView.GetRowCellValue(e.RowHandle, "Model_Name").Equals("TOTAL") || !bgGrdView.GetRowCellValue(e.RowHandle, "Model_Name").Equals("G-TOTAL"))
                {
                    if (bgGrdView.Columns[e.Column.ColumnHandle].FieldName.Contains("Qty"))
                    {
                ...
                if (bgGrdView.GetRowCellValue(e.RowHandle, "Model_Name").Equals("TOTAL") && e.Column.ColumnHandle > 0)
                {
